...
fn extract_api_key_from_provider(provider: &crate::cc_switch::provider::Provider) -> Option<String> {
...
fn extract_base_url_from_provider(provider: &crate::cc_switch::provider::Provider) -> Option<String> {
    if let Some(env) = provider.settings_config.get("env") {
