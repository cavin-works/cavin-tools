  ...
  return (
    ...
    >
                ...
                >
                  <div className={`flex-shrink-0 ${sidebarCollapsed ? '' : 'w-9 h-9 rounded-lg flex items-center justify-center'} ${
                    isActive
                      ...
                      : sidebarCollapsed ? '' : 'bg-sidebar-accent'
                  }`}>
                  ...
                  {!sidebarCollapsed && (
                    <div className="flex-1 min-w-0">
                      <div className={`text-sm font-medium ${isActive ? 'text-sidebar-primary-foreground' : 'text-sidebar-foreground'}`}>
                        {tool.name}
                      </div>
                      <div className={`text-xs truncate max-w-[140px] ${isActive ? 'text-sidebar-primary-foreground/80' : 'text-muted-foreground'}`}>
      ...
      <div className="flex-shrink-0 p-3 space-y-3">
        ...
        {!sidebarCollapsed && (
          <div className="flex items-center gap-2 px-3 py-2 text-sm text-muted-foreground">
            <div className={`w-2 h-2 rounded-full ${
              theme === 'dark' ? 'bg-primary' : theme === 'light' ? 'bg-chart-1' : 'bg-chart-2'
            }`} />
            <span>{themeText}</span>
          </div>
        )}
        ...
        {/* 底部图标按钮组 */}
        <div className={`flex items-center ${sidebarCollapsed ? 'flex-col gap-1' : 'justify-center gap-1'} border-t border-sidebar-border pt-3`}>
          {/* 主题切换 */}
          <button
            onClick={cycleTheme}
